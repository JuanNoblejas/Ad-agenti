...
class HistoryAdapter :
    ...
    companion object {
        private val COLOR_ENTRY = Color.parseColor("#2E7D32") // Green
        private val COLOR_EXIT = Color.parseColor("#E65100")  // Orange
    }
    ...
    inner class ViewHolder(
        ...
        fun bind(record: GpsRecord) {
            val isEntry = record.type == "ENTRY"

            // Set event type text with emoji
            binding.tvEventType.text = if (isEntry) {
                "🏠 Entry"
            } else {
                "🚶 Exit"
            }
            ...
            binding.ivIcon.setImageResource(
                if (isEntry) R.drawable.ic_enter else R.drawable.ic_exit
            )
            ...
            val textColor = if (isEntry) COLOR_ENTRY else COLOR_EXIT
