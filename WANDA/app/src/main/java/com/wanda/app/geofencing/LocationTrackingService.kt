...
class LocationTrackingService : Service() {
    ...
    private fun startForegroundNotification() {
        val notification = NotificationCompat.Builder(this, "tracking_channel")
            ...
            .setContentTitle("WANDA - Tracking active")
    ...
    private fun setupGeofence() {
        ...
        geofenceHelper.removeGeofence(
            onSuccess = {
                Log.d(TAG, "Previous geofence removed, adding new one...")
                geofenceHelper.addGeofence(
                    lat = latitude,
                    lng = longitude,
                    radius = radius,
                    onSuccess = {
                        Log.d(TAG, "Geofence added at ($latitude, $longitude) radius: $radius m")
                    },
                    onFailure = { exception ->
                        Log.e(TAG, "Error adding geofence: ${exception.message}", exception)
                    }
                )
            },
            onFailure = {
